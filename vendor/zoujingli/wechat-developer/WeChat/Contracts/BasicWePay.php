...
class BasicWePay
{
    ...
    protected function callPostApi($url, array $data, $isCert = false, $signType = 'HMAC-SHA256', $needSignType = true)
    {
        ...
        if ($isCert) {
            ...
            $option['ssl_key'] = Tools::pushFile($this->config->get('mch_id'). 'pkey.pem', $option['ssl_key']);
            $option['ssl_cer'] = Tools::pushFile($this->config->get('mch_id'). 'cer.pem', $option['ssl_cer']);
