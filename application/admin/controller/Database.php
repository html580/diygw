...
class Database extends Admin {
	...
	public function importfiles(){
        $this->assign('meta_title',"安装应用");
        $filename = input("filename", '');
        $del = input("del", '');
        $dirPath = APP_PATH . \think\Request::instance()->module() . '/' . 'data';
        if(empty($filename)){
            $flag = \FilesystemIterator::KEY_AS_FILENAME;
            $glob = new \FilesystemIterator($dirPath, $flag);
            $list = array();
            foreach ($glob as $name => $file) {
                $extension        = strtoupper(pathinfo($file->getFilename(), PATHINFO_EXTENSION));
                if($extension=="SQL"){
            ...
            $filePath = $dirPath. '/'.$filename.'.sql';
            array_map("unlink", glob($filePath));
            if (count(glob($filePath))) {
                return $this->error('应用删除失败，请检查权限！');
            } else {
                return $this->success('应用文件删除成功！');
            }
        }else{
            $filePath = $dirPath. '/'.$filename.'.sql';
