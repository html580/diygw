...
class ToolsService
{
    ...
    public static function getCurrentMenu($list, $value, $key = 'id')
    {
        if (!$value) {
            return [];
        }
        $node=[];
        foreach ($list as $vo){
            if (strtolower($vo[$key]) == strtolower($value)) {
                return $vo;
            }
        }
        return $node;
    ...
    public static function getTopParentMenu($list, $id = 0, $key = 'id', $pkey = 'pid')
    {
        if (!$id) {
            return [];
        }
        $node=[];
        foreach ($list as $vo){
            if (intval($vo[$pkey]) == 0 && intval($vo[$key]) === intval($id)) {
                $node = $vo;
                break;
            }
            if (intval($vo[$pkey]) > 0 && intval($vo[$key]) === intval($id)) {
                return self::getTopParentMenu($list,intval($vo[$pkey]), $key, $pkey);
            }
        }
        return $node;
    }
