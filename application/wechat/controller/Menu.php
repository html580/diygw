...
class Menu extends BasicAdmin
{
    ...
    {
        return parent::_list(Db::name($this->table)->where('mpid', $this->mpid), false, true);
    }
    ...
    private function _push()
    {
        ...
        $result = (array)Db::name($this->table)->field($field)->where($map)->where('mpid', $this->mpid)->order('sort ASC,id ASC')->select();
