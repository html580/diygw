...
/*if (typeof layui !== 'undefined') {
    ...
    $.msg = new function () {
        ...
        this.auto = function (data, time) {
            return (parseInt(data.code) === 1) ? self.success(data.msg, time, function () {
                ...
                self.dialogIndexs = [];
            }) : self.error(data.msg, 5, function () {
    ...
    if ($.support.pjax) {
        ...
        $(document).pjax('a[target!=_blank][target!=_self]', '.framework-body');
        ...
        $(document).on('submit', 'form', function (event) {
            //隐藏返回值
            $.pjax.submit(event, '.framework-body', {push: false});
        });
        */
    ...
    $.menu = new function () {
        ...
        this.queryNode = function (url) {
            ...
            var node = url.replace(/.*spm=([\d\-m]+).*/ig, '$1');
            if (!/^m\-/.test(node)) {
        ...
        // 后台菜单动作初始化
        this.selectNode=function (url) {
            var self = this;
            var node = self.queryNode(url||self.getUri());
            if (/^m\-/.test(node)) {
    ...
    $body.on('click', '[data-submenu-layout]>a', function (e) {
    ...
    /*! 注册 data-check 事件行为 */
    $body.on('click', '[data-check-target]', function (e) {
        e.preventDefault();
        e.stopPropagation();
    ...
    $body.on('click', '[data-update]', function (e) {
        ...
        $.msg.confirm($(this).attr('data-message')||'确定要操作这些数据吗？', function () {
    ...
    $body.on('click', 'a[data-page-href]', function (e) {
        e.preventDefault();
        e.stopPropagation();
    ...
    /*! 注册 data-icon 事件行为 */
    $body.on('click', '[data-icon]', function (e) {
        e.preventDefault();
        e.stopPropagation();
    ...
    $body.on('click', '[data-tips-image]', function (e) {
        e.preventDefault();
        e.stopPropagation();
    ...
    $body.on('mouseenter', '[data-tips-text]', function (e) {
        e.preventDefault();
        e.stopPropagation();
    ...
    $body.on('click', '[data-phone-view]', function (e) {
        e.preventDefault();
        e.stopPropagation();
    ...
    /*! 分页事件切换处理 */
    $body.on('change', '.pagination-trigger select', function (e) {
        e.preventDefault();
        e.stopPropagation();
