...
class Fans extends BasicAdmin
{
    ...
    {
        $tags = Db::name('WechatFansTags')->where('mpid', $this->mpid)->column('id,name');
        foreach ($list as &$vo) {
    ...
    public function backadd()
    {
        try {
            ...
            Db::name($this->table)->where('mpid', $this->mpid)->whereIn('openid', $openids)->setField('is_black', '1');
        } catch (\Exception $e) {
    ...
    public function tagset()
    {
        ...
        $fans = Db::name('WechatFans')->where('mpid', $this->mpid)->where(['id' => $fans_id])->find();
        ...
        try {
            ...
            Db::name('WechatFans')->where('mpid', $this->mpid)->where(['id' => $fans_id])->setField('tagid_list', $tags);
        } catch (\Exception $e) {
    ...
    private function _getActionOpenids()
    {
        ...
        $openids = Db::name($this->table)->where('mpid', $this->mpid)->whereIn('id', explode(',', $ids))->column('openid');
    ...
    {
        try {
            Db::name($this->table)->where('1=1')->where('mpid', $this->mpid)->delete();
