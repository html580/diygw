...
class News extends BasicAdmin
{
    ...
    {
        $this->title = '微信图文列表';
        $db = Db::name($this->table)->where('mpid', $this->mpid)->where(['is_deleted' => '0']);
    ...
    public function image()
    {
        ...
        return $this->_list(Db::name('WechatNewsMedia')->where('mpid', $this->mpid)->where('type', 'image'));
    }
    ...
    public function add()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if (($ids = $this->_apply_news_article($data['data'])) && !empty($ids)) {
                $post = ['article_id' => $ids, 'create_by' => session('user.id'),'mpid'=>$this->mpid];
    ...
    public function edit()
    {
        ...
        if (!empty($ids)) {
            $post = ['id' => $id, 'article_id' => $ids, 'create_by' => session('user.id'),'mpid'=>$this->mpid];
    ...
    public function del()
    {
        if (DataService::update($this->table,['mpid'=>$this->mpid])) {
            $this->success("图文删除成功!", '');
        }
    ...
    public function push()
    {
        ...
        switch (strtolower($this->request->get('action', ''))) {
            case 'getuser':
                ...
                $list = $db->where('mpid', $this->mpid)->where(['subscribe' => '1'])->limit(200)->column('nickname');
                foreach ($list as &$vo) {
            ...
            default :
                ...
                // Get 请求，显示选择器界面
                if ($this->request->isGet()) {
                    $fans_tags = (array)Db::name('WechatFansTags')->where('mpid', $this->mpid)->select();
                    $count = Db::name('WechatFans')->where('mpid', $this->mpid)->where(['subscribe' => '1'])->count();
