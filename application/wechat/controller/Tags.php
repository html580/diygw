...
class Tags extends BasicAdmin
{
    ...
    {
        $this->title = '微信粉丝标签管理';
        list($get, $db) = [$this->request->get(), Db::name($this->table)->where('mpid', $this->mpid)];
        foreach (['name'] as $key) {
    ...
    public function add()
    {
        if ($this->request->isGet()) {
            return parent::_form($this->table, 'form', 'id',['mpid'=>$this->mpid],['mpid'=>$this->mpid]);
        }
        $name = $this->request->post('name', '');
        empty($name) && $this->error('粉丝标签名不能为空!');
        if (Db::name($this->table)->where('mpid', $this->mpid)->where('name', $name)->count() > 0) {
    ...
    public function edit()
    {
        ...
        $name = $this->request->post('name', '');
        $info = Db::name($this->table)->where('mpid', $this->mpid)->where(['name' => $name])->find();
        if (!empty($info)) {
        ...
        try {
            ...
            DataService::save($this->table, ['id' => $id, 'name' => $name,'mpid' =>$this->mpid], 'id');
        } catch (\Exception $e) {
    ...
    {
        Db::name($this->table)->where('mpid', $this->mpid)->where('1=1')->delete();
        if (TagsService::sync()) {
